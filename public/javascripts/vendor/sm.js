...
+function($) {
    ...
    Router.prototype._switchToDocument = function(url, ignoreCache, isPushState, direction) {
        ...
        if (cacheDocument) {
             ...
             * 执行已缓存的js
             */
            cacheDocument.$content.find('script').each(function (i, n) {
                var $n = $(n)
                var scriptSrc = $n.attr('src')
                /*var actionScript = sessionStorage.getItem(scriptSrc)
                if (actionScript && scriptSrc && scriptSrc.indexOf('shared') === -1) {
                    eval(actionScript)
                }*/
                if (scriptSrc && scriptSrc.indexOf('shared') === -1) {
                   $.get(scriptSrc, function (res) {
                        if (res) {
                            try {
                                loadScriptArray.push(scriptSrc)
                                //eval(res)
                                sessionStorage.setItem(scriptSrc, res)
                            } catch (e) {
                                alert(e)
                                console.log(e)
                            } 
            ...
            this._loadDocument(url, {
                success: function($doc) {
                     ...
                     * 异步载入js并执行
                     */
                    $doc.find('script').each(function (i, n) {
                        var $n = $(n)
                        var scriptSrc = $n.attr('src')
                        if (loadScriptArray.indexOf(scriptSrc) === -1) {
                            if (scriptSrc && scriptSrc.indexOf('shared') === -1) {
                               $.get(scriptSrc, function (res) {
                                    if (res) {
                                        try {
